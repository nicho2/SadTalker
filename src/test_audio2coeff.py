...
class Audio2Coeff():
    ...
    def generate(self, batch, coeff_save_dir, pose_style, ref_pose_coeff_path=None):
        ...
        with torch.no_grad():
            ...
            savemat(os.path.join(coeff_save_dir, '%s__%s.mat'%(batch['pic_name'], batch['audio_name'])),
                    {'coeff_3dmm': coeffs_pred_numpy})

            return os.path.join(coeff_save_dir, '%s__%s.mat'%(batch['pic_name'], batch['audio_name']))
